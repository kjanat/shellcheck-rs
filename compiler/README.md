...
`mise run lower:specialize` against the canonical `-O1` dump reports **369 refusals over 108 bindings in 37 modules**. The groups below are by the work each needs, not by package.

| What it needs                                                                                                                                           | Refusals | Where                                                                                                                                                                                                                                                               |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`containers`.** `Set` and `Map` balancing, linking, equality and forcing.                                                                             |       79 | `Data.Set.Internal` 50 (`balanceR` 13, `link` 13, `insertMax` 8, `balanceL` 6), `Data.Map.Internal` 29 (`balanceL` 11, `insertMax` 6, `balanceR` 4)                                                                                                                 |
...
| **List and string functions whose bodies the dump does not contain.**                                                                                   |       33 | `base:Data.OldList` 16 (`dropLengthMaybe` 6, `lines` 3, `nubBy` 2, `unlines` 2, `groupBy`, `sortBy`, `words`), `base:GHC.List` 10 (`$wbreak` 3, `lookup` 2, `$w!!`, `$wspan`, `flipSeq`, `init1`, `zip`), `base:Data.Maybe.mapMaybe` 5, `base:Data.List.NonEmpty` 2 |
| **`Eq`/`Ord` for lists, tuples and `Int`.**                                                                                                             |       29 | `ghc-prim:GHC.Classes` (`$fEqList_$s$c==1` 6, `$fOrdList_$ccompare` 5, `eqInt` 4, `$fEqList_$c==` 3, `$fEqList_$s$c==2` 2, `compareInt` 2, `compareInt#` 2, five more at 1)                                                                                         |
| **`text`.**                                                                                                                                             |       27 | `Data.Text.Show.$wunpackCStringAscii#`                                                                                                                                                                                                                              |
| **Primops with no implementation yet.**                                                                                                                 |       26 | `ghc-prim:GHC.Prim` 22 (`leWord#` 9, `uncheckedIShiftRA#` 8, `unsafeFreezeArray#` 2, `andI#`, `orI#`, `quotRemInt#`), `ghc-prim:GHC.Magic.runRW#` 4                                                                                                                 |
...
| **Unicode character tables.** `isSpace`, `toSimpleLowerCase` and `toSimpleUpperCase` are also passed as values.                                         |       17 | `SimpleLowerCaseMapping` 9 (`toSimpleLowerCase` 8, `$wtoSimpleLowerCase` 1), `base:GHC.Unicode` 7 (`isSpace` 4, `$wisAlpha` 2, `isAlpha` 1), `SimpleUpperCaseMapping.toSimpleUpperCase` 1                                                                           |
...
| **`Show`.**                                                                                                                                             |       15 | `base:GHC.Show` 14 (`$w$cshowsPrec15` 10, `$fShow(,)_itos'` 3, `$wshowSignedInt` 1), `base:Data.Version.$wshowVersion` 1                                                                                                                                            |
...
Every count is a lower bound. A refused instance never revealed its own requirements, which is why clearing a blocker can raise other counts. After the list predicates, `map` rose from 24 to 29 and `System.Info.os` appeared. After `compare`, the `Map` code behind string-keyed lookups became reachable. After pointer equality, the `Set` and `Map` balancing code behind it did. After the list functions, the functions passed to them did: `fst`, `snd`, `isSpace` and `toSimpleLowerCase`. After substitution learned to apply a type constructor, the transformer code did, and `containers` rose again.
...
```
Refused: 930 = 571 at a closed signature + 359 at an open signature
Open signatures: 359 refused instances of 359 bindings quantified over types they were not given; 43 of those bindings were also requested at closed types
```

For 316 of the 359, the survey never requested a closed instantiation. Some are artifacts of the rooting, but not all of them. `ShellCheck.CFGAnalysis`'s `ST` code is quantified over the state thread `s`, which `runST`'s rank-2 type never instantiates to a closed type, so no specialization can close it. Those are real blockers.
