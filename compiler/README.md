...
`mise run canary` runs a complete small pipeline: compile the real Haskell sources in `compiler/canary/` with GHC 9.6.7 and the format-6 plugin; lower the selected pure entries and every dependency; emit standalone Rust; compile with `rustc`; compare against the Haskell oracle. Both `-O1` and `-O0 -fmax-simplifier-iterations=0` check 1,331 entry/input combinations, each with optimized and overflow-checked Rust: 5,324 comparisons. Signed 64-bit boundaries exercise overflowing arithmetic. Standard output, standard error and successful exit are checked. Cross-module calls, shared strict intermediates, six comparisons, literal cases, nested branches and boxed Int construction, forcing, passing and return are covered.
...
`compiler/canary/boxed_checks.rs` compiles against the actual generated Rust in both profiles. Its five semantic tests use deferred/instrumented inputs to check unused arguments, unselected branches, delayed calls, one-time evaluation of shared inputs, forcing of unused constructor fields and CAF cell identity. The embedded runtime's five tests also execute in each of the five generated modules (30 test executions per profile).
