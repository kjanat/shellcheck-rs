...
`mise run lower:specialize` against the canonical `-O1` dump reports **411 refusals over 103 bindings in 35 modules**. The groups below are by the work each needs, not by package.
...
| **Primops with no implementation yet.**                                                                                                                 |       92 | `ghc-prim:GHC.Prim` 88 (`dataToTag#` 39, `reallyUnsafePtrEquality#` 28, `uncheckedIShiftRA#` 8, `leWord#` 5, `tagToEnum#` 3, `unsafeFreezeArray#` 2, `andI#`, `orI#`, `quotRemInt#`), `ghc-prim:GHC.Magic.runRW#` 4                                                                                              |
...
| **`containers`.** `Set` and `Map` balancing, equality and forcing.                                                                                      |       33 | `Data.Set.Internal` 20 (`insertMax` 8, `link` 5), `Data.Map.Internal` 13 (`balanceL` 3, `insertMax` 3, `$fEqMap_$c==` 2, `balanceR` 2)                                                                                                                                                                           |
| **`text`.**                                                                                                                                             |       27 | `Data.Text.Show.$wunpackCStringAscii#`                                                                                                                                                                                                                                                                           |
| **`Eq`/`Ord` for lists, tuples and `Int`.**                                                                                                             |       22 | `ghc-prim:GHC.Classes` (`$fOrdList_$ccompare` 5, `$fEqList_$s$c==1` 3, `eqInt` 3, `compareInt` 2, `compareInt#` 2, seven more at 1)                                                                                                                                                                              |
...
| **`Integer`.** This backend has no carrier for arbitrary precision.                                                                                     |       14 | `ghc-bignum:GHC.Num.Integer` (`integerCompare` 4, `integerEq` 3, `integerAdd` 2, `integerGe` 2, three more at 1)                                                                                                                                                                                                 |
...
Every count is a lower bound. A refused instance never revealed its own requirements, which is why clearing a blocker can raise other counts. After the list predicates, `map` rose from 24 to 29 and `System.Info.os` appeared. After `compare`, the `Map` code behind string-keyed lookups became reachable, and it brought 7 more `reallyUnsafePtrEquality#`, 6 more `containers` refusals and 4 `integerCompare`.
...
```
Refused: 1381 = 1022 at a closed signature + 359 at an open signature
