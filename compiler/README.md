...
A library binding without an unfolding in the dump had to be implemented in `h2r-rt` by hand, as `map` and `eqString` were. That does not scale to `Set.balanceR` or `Map.link`, and rewriting library algorithms is what this compiler must not do. The library's own Core scales. `containers` is now rebuilt from its Hackage source with the plugin, and its dumps load into the same world as ShellCheck's.

`mise run extract:libraries` runs `compiler/extract-library.sh containers`, which writes `compiler/library-json/containers/`. `h2r lower --with <dir>` loads a library directory beside the program's, the `lower:*` tasks and the canary load `containers` that way, and the M1 to M3a reports keep measuring the program alone.

The dumps have to name every binding exactly as ShellCheck's Core refers to it, `$containers-0.6.7$Data.Set.Internal$balanceR`, so the library is compiled as the unit the program linked:

- **The module list** is the installed package's own 37 modules, from `ghc-pkg field containers exposed-modules,hidden-modules`. These are the modules the program was linked against.
- **The unit id.** GHC cannot compile a home unit whose id is one the loaded plugin depends on, and `h2r-plugin` depends on `containers` through `aeson`. With `-this-unit-id containers-0.6.7` the compilation deadlocks before its first module, at 0% CPU. The build uses the stand-in id `containers-h2r`, and the plugin's new `unit=` option names the dump after the installed unit. It rewrites every stable name of the home unit, in values and in object keys (the id table is keyed by stable name), and the string literal GHC writes the unit id as, in each `SrcLoc`'s package and in `$trModule`.
...
- **The source path.** GHC writes the path a module was compiled from into the `SrcLoc` of every `HasCallStack` call site, and those appear in `error`'s output. The installed `libHScontainers-0.6.7.a` embeds `libraries/containers/containers/src/...`, hadrian's path, so the script compiles from a root in which the source sits at that relative path.
- **Flags.** `-O2 -XHaskell2010`, from `containers.cabal`.
...
- `errorUnusedArgument`, `errorUnusedLet` and `errorUnusedShared` bind an `error` and never demand it. They were refusal fixtures while `error` had no implementation, and now run as differential fixtures. `undefinedUnused` keeps the refusal for a call with demand evidence alone.
...
`mise run lower:specialize` against the canonical `-O1` dump reports **305 refusals over 92 bindings in 36 modules**, with `containers` compiled into the world. The groups below are by the work each needs, not by package.

| What it needs                                                                                                                                           | Refusals | Where                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Foreign libraries.** Haskell packages not yet compiled into the world.                                                                                |       70 | `regex-tdfa` 58 (`Text.Regex.TDFA.String.compile` 49, `Text.Regex.TDFA.NewDFA.Tester.matchTest_multi5` 9), `aeson` 4, `fgl` 3, `filepath` 3, `parsec` 2                                                                                                                                       |
| **`Eq`/`Ord` for lists, tuples and `Int`.** `ghc-prim`'s class methods and dictionaries.                                                                |       44 | `ghc-prim:GHC.Classes` (`$fOrdList_$ccompare` 14, `$fEqList_$c==` 6, `$fEqList_$s$c==1` 6, `eqInt` 4, `$fEqList_$s$c==2` 2, `$fEqList_$s$fEqList1` 2, `compareInt` 2, `compareInt#` 2, six more at 1)                                                                                         |
| **List and string functions whose bodies the dump does not contain.**                                                                                   |       36 | `base:Data.OldList` 17 (`dropLengthMaybe` 6, `lines` 3, `nubBy` 2, `unlines` 2, `groupBy`, `intercalate_$spoly_go1`, `sortBy`, `words`), `base:GHC.List` 10 (`$wbreak` 3, `lookup` 2, `$w!!`, `$wspan`, `flipSeq`, `init1`, `zip`), `base:Data.Maybe.mapMaybe` 7, `base:Data.List.NonEmpty` 2 |
| **`text`.**                                                                                                                                             |       27 | `Data.Text.Show.$wunpackCStringAscii#`                                                                                                                                                                                                                                                        |
| **`Integer`.** This backend has no carrier for arbitrary precision.                                                                                     |       20 | `ghc-bignum:GHC.Num.Integer` (`integerEq` 8, `integerCompare` 5, `integerAdd` 2, `integerGe` 2, three more at 1)                                                                                                                                                                              |
| **Unicode character tables.** `isSpace`, `toSimpleLowerCase` and `toSimpleUpperCase` are also passed as values.                                         |       18 | `SimpleLowerCaseMapping` 9 (`toSimpleLowerCase` 8, `$wtoSimpleLowerCase` 1), `base:GHC.Unicode` 8 (`isSpace` 4, `$wisAlpha` 2, `isAlpha`, `$wisAlphaNum`), `SimpleUpperCaseMapping.toSimpleUpperCase` 1                                                                                       |
| **`Show`.**                                                                                                                                             |       18 | `base:GHC.Show` 17 (`$w$cshowsPrec15` 10, `$fShow(,)_itos'` 3, `itos` 3, `$wshowSignedInt` 1), `base:Data.Version.$wshowVersion` 1                                                                                                                                                            |
| **Monad transformer dictionaries.** `Applicative`, `Functor` and `Monad` for `StateT`, `WriterT` and `RWST`, and `MonadReader`/`MonadWriter` over them. |       17 | `transformers` 14, `mtl` 3                                                                                                                                                                                                                                                                    |
| **Other `base` class dictionaries.**                                                                                                                    |       14 | `$fApplicativeMaybe` 9, `Foldable []` 2, `pure @[]` 1, `Monoid (a, b, c, d)` 1, `(+) @Int` 1                                                                                                                                                                                                  |
| **Library functions passed as values.** An external is implemented at a saturated call; these sites pass one to `map` or `dropWhile`.                   |       11 | `base:Data.Tuple` 9 (`snd` 6, `fst` 3), `base:GHC.Base.++` 2                                                                                                                                                                                                                                  |
| **`Read`.**                                                                                                                                             |       10 | `base:Text.Read` 8 (`readEither2` 4, `readEither5` 4), `base:Text.ParserCombinators.ReadP.run` 2                                                                                                                                                                                              |
| **Primops with no implementation yet.**                                                                                                                 |        9 | `ghc-prim:GHC.Prim` 5 (`unsafeFreezeArray#` 2, `andI#`, `orI#`, `quotRemInt#`), `ghc-prim:GHC.Magic.runRW#` 4                                                                                                                                                                                 |
...
```
Refused: 833 = 474 at a closed signature + 359 at an open signature
Open signatures: 359 refused instances of 359 bindings quantified over types they were not given; 60 of those bindings were also requested at closed types
```

For 299 of the 359, the survey never requested a closed instantiation. Some are artifacts of the rooting, but not all of them. `ShellCheck.CFGAnalysis`'s `ST` code is quantified over the state thread `s`, which `runST`'s rank-2 type never instantiates to a closed type, so no specialization can close it. Those are real blockers.
...
| cause                                         | refusals |
| --------------------------------------------- | -------: |
| a let binds a value whose type has no carrier |       23 |
| a let group binds several values at once      |        2 |
| **a recursive value binding**                 |    **0** |
| **a join point bound as a value**             |    **0** |

Of the 23, 1 is at a closed signature (`aeson`'s `Encoding'`) and 22 at an open one (`Monad` 11, `MonadState` 6, a `forall` 5). None of them is about recursion. A conflated message cannot be ranked, and until it was split the ranking could not say which of the three the program actually contains.
