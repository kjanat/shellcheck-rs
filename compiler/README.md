...
`mise run canary` now runs a complete small pipeline: compile the real Haskell sources in `compiler/canary/` with GHC 9.6.7 and the format-6 plugin; lower the selected pure entries and every dependency; emit standalone Rust; compile with `rustc`; compare 350 entry/input combinations in both optimized and overflow-checked Rust builds (700 comparisons). Signed 64-bit boundaries exercise overflowing addition, subtraction and multiplication. Standard output, standard error and successful exit are checked. The helper lives in a separate Haskell module, so this exercises actual cross-module linkage as well as argument order and literals. Composed arithmetic, nested cross-module calls and a shared strict case intermediate exercise multi-step evaluation.
...
The primitive table recognizes only exact external `GHC.Prim` identities for `+#`, `-#` and `*#`, after lexical resolution, with matching `[PrimOp]` metadata and arity. Their fixed `Int# -> Int# -> Int#` signatures are checked against operands and result. NIR records `IntArithmetic` with ordered operands and source origin; the independent source verifier rejects changed operators, operands and origins. Rust uses `wrapping_add`, `wrapping_sub` and `wrapping_mul`, not build-mode-dependent arithmetic. Nested Int# applications are evaluated in argument order. Strict Int# cases with exactly one DEFAULT alternative bind the evaluated scrutinee once, represented by a source-attributed move; even an unused case result retains its scrutinee evaluation. The verifier independently checks each subtree, its complete instruction slice, binder scope and node accounting. Cases with branches, alternative binders or non-Int# types remain refused, as do computed lifted arguments. Partial applications, primitive type arguments and other primitives remain refused; no general external-call fallback is introduced.
