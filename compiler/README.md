...
`mise run lower:specialize` against the canonical `-O1` dump reports **598 refusals over 102 bindings in 34 modules**. The groups below are by the work each needs, not by package.

| What it needs                                                                                                                                                             | Refusals | Where                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **List and string functions whose bodies the dump does not contain.** Either a wider dump, or implementations against their exact GHC signatures the way `(++)` was done. |      271 | `base:GHC.Base` 118 (`eqString` 84, `map` 24, `++_$s++` 9, `++` 1), `base:GHC.List` 109 (`elem` 74, `reverse1` 9, `dropWhile` 8, `$wlenAcc` 6, `takeWhile` 4, `filter` 2, six more at 1), `base:Data.OldList` 41 (`isPrefixOf` 32, `lines` 3, `unlines` 2, four more at 1), `base:Data.Maybe.mapMaybe` 2, `base:Data.List.NonEmpty` 1 |
| **Primops with no implementation yet.**                                                                                                                                   |       84 | `ghc-prim:GHC.Prim` 80 (`dataToTag#` 38, `reallyUnsafePtrEquality#` 21, `uncheckedIShiftRA#` 8, `leWord#` 5, `tagToEnum#` 3, `unsafeFreezeArray#` 2, `andI#`, `orI#`, `quotRemInt#`), `ghc-prim:GHC.Magic.runRW#` 4                                                                                                                   |
...
| **`Eq`/`Ord` for lists, tuples and `Int`.**                                                                                                                               |       56 | `ghc-prim:GHC.Classes` (`$fOrdList_$s$ccompare1` 39, `$fOrdList_$ccompare` 5, `compareInt#` 2, `eqInt` 2, eight more at 1)                                                                                                                                                                                                            |
| **`containers`.** `Set` and `Map` balancing, equality and forcing.                                                                                                        |       27 | `Data.Set.Internal` 20 (`insertMax` 8, `link` 5), `Data.Map.Internal` 7                                                                                                                                                                                                                                                               |
...
| **`Show`.**                                                                                                                                                               |       11 | `base:GHC.Show` 10 (`$w$cshowsPrec15` 7, `$fShow(,)_itos'` 2), `base:Data.Version.$wshowVersion` 1                                                                                                                                                                                                                                    |
| **`Read`.**                                                                                                                                                               |       10 | `base:Text.Read` 8 (`readEither2` 4, `readEither5` 4), `base:Text.ParserCombinators.ReadP.run` 2                                                                                                                                                                                                                                      |
| **`Integer`.** This backend has no carrier for arbitrary precision.                                                                                                       |        8 | `ghc-bignum:GHC.Num.Integer`                                                                                                                                                                                                                                                                                                          |
...
| **Everything else.**                                                                                                                                                      |        2 | `base:GHC.Base.id`, `base:GHC.Base.maxInt`                                                                                                                                                                                                                                                                                            |

Every count is a lower bound. A refused instance never revealed its own requirements, which is why clearing unboxed tuples raised the boundary from 450 to 592 and put `text` on it. The largest row is 271 refusals of ordinary list code in `base`, and the survey cannot decide whether to answer it by widening the dump or by implementing the signatures.
...
```
Refused: 1548 = 1188 at a closed signature + 360 at an open signature
Open signatures: 360 refused instances of 360 bindings quantified over types they were not given; 19 of those bindings were also requested at closed types
```

Only 6 quantified bindings lower at their open signature. For 341 of the 360, the survey never requested a closed instantiation. Some are artifacts of the rooting, but not all of them. `ShellCheck.CFGAnalysis`'s `ST` code is quantified over the state thread `s`, which `runST`'s rank-2 type never instantiates to a closed type, so no specialization can close it. Those are real blockers.
...
| cause                                         | refusals |
| --------------------------------------------- | -------: |
| a let binds a value whose type has no carrier |       80 |
| a let group binds several values at once      |        1 |
| **a recursive value binding**                 |    **0** |
| **a join point bound as a value**             |    **0** |

Of the 80, 57 are at a closed signature (`WriterT` 45, `RWST` 11, `aeson`'s `Encoding'` 1) and 23 at an open one (`Monad` 10, `MonadState` 6, a `forall` 5, `RWST` 1, a function 1). None of them is about recursion. A conflated message cannot be ranked, and until it was split the ranking could not say which of the three the program actually contains.
