...
The [residual-laziness census](#m2--who-receives-the-lazy-arguments) leaves **294** class-op *argument* sites in the unresolved tier. Each is an argument of exactly one population site, and the mapping is asserted: **294 of 294 map**, on all seven dumps. The other 271 population sites are class-op applications the census never counted, because none of their arguments is a non-trivial computation in a lazy position.
...
One thing the dump cannot answer: **which field of a dictionary a selector reads**. Class-op selectors are globals, and format 5 carries no type and no unfolding for a global, so neither the selector's type (`C a => …`) nor its `case d of C:C … m … -> m` body is available. The field order is therefore asserted per class — 17 classes, in the style of the [list axioms](#the-axiom-layer) — and **every use of an entry is cross-checked against that dictionary constructor's own `repArity` in the dump**. Over all seven dumps the check reports **0 disagreements**, and **0** sites fall outside the table.
...
Historical comparison, canonical/`-O1` only (source: [M2.2 — Accounting](#accounting-1), the `M2.2 accounting` block):
...
The canary separately probes forced `errorWithoutStackTrace` calls with plain, empty, Unicode and multiline messages, plus an unboxed-result entry, in both profiles. It checks the GHC oracle's exact stderr (`<program>: <message>` followed by a newline), empty stdout and exit 1, while asserting that Rust emission remains refused. These ten oracle/refusal probes are not included in the generated-code differential count. The Unicode probe requires a UTF-8 environment. Implementing the message evaluation and exception path remains the next backend step.
...
The closing deliverable, measured rather than estimated: **592 refusals over 103 bindings in 35 modules**, from `mise run lower:specialize` against the canonical `-O1` dump. Grouped by the work each group needs rather than by package, because the package is not what decides the effort.

| What it needs                                                                                                                                                                             | Refusals | Where                                                                                                                                                                                                                               |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **List and string functions with ordinary Haskell bodies the dump does not contain.** Either a wider dump, or implementations against their exact GHC signatures the way `(++)` was done. |      272 | `base:GHC.Base` 125 (`eqString` 79, `map` 24, `++_$s++` 8), `base:GHC.List` 107 (`elem` 72, `reverse1` 9, `dropWhile` 8, `$wlenAcc` 6, `takeWhile` 4, `filter` 2), `base:Data.OldList` 40 (`isPrefixOf` 31, `lines` 3, `unlines` 2) |
| **Primops with no implementation yet.** Each is a machine operation, not a library.                                                                                                       |       84 | `ghc-prim:GHC.Prim` 80 (`dataToTag#` 38, `reallyUnsafePtrEquality#` 21, `uncheckedIShiftRA#` 8, `leWord#` 5, `tagToEnum#` 3), `ghc-prim:GHC.Magic` 4 (`runRW#`)                                                                     |
| **`Eq`/`Ord` dictionaries for lists and tuples.** Derivable once their element comparisons are.                                                                                           |       56 | `ghc-prim:GHC.Classes` (`$fOrdList_$s$ccompare1` 39, `$fOrdList_$ccompare` 5)                                                                                                                                                       |
| **Genuinely foreign libraries.** Nothing in this backend substitutes for them.                                                                                                            |       65 | `regex-tdfa` 53 (`Text.Regex.TDFA.String.compile` 49), `aeson` 4, `fgl` 3, `filepath` 3, `parsec` 2                                                                                                                                 |
| **`containers`.** `Set` and `Map` balancing and equality.                                                                                                                                 |       27 | `Data.Set.Internal` 20 (`insertMax` 8, `link` 5), `Data.Map.Internal` 7                                                                                                                                                             |
| **`text`.** New to the ranking: nothing reached it until the unboxed-tuple work cleared what stood in front.                                                                              |       27 | `Data.Text.Show.$wunpackCStringAscii#`                                                                                                                                                                                              |
| **Monad transformer dictionaries.** `Applicative`/`Functor`/`Monad` for `StateT`, `WriterT`, `RWST`, and `MonadReader`/`MonadWriter` over them.                                           |       16 | `transformers` 13, `mtl` 3                                                                                                                                                                                                          |
| **`Show`.**                                                                                                                                                                               |       10 | `base:GHC.Show` (`$w$cshowsPrec15` 7)                                                                                                                                                                                               |
| **`Integer`.** Arbitrary precision, which this backend has no carrier for.                                                                                                                |        9 | `ghc-bignum:GHC.Num.Integer` 8, `base:GHC.Num` 1                                                                                                                                                                                    |
| **IO.** The next milestone's subject.                                                                                                                                                     |        8 | `base:GHC.IO.Handle.Text` 6 (`hPutStr2`), `base:System.Environment` 1, `base:System.Console.GetOpt` 1                                                                                                                               |
| **Error behaviour still outstanding.** `patError` needs the pattern-failure message; the rest of the family now lowers as a dead end.                                                     |        6 | `base:Control.Exception.Base`                                                                                                                                                                                                       |
| **Unicode character tables.**                                                                                                                                                             |        3 | `base:GHC.Unicode` 2, `GHC.Unicode.Internal.Char.UnicodeData.SimpleLowerCaseMapping` 1                                                                                                                                              |
| **Everything else,** one or two each.                                                                                                                                                     |        9 | `base:Data.Foldable`, `base:Data.Maybe`, `base:Text.ParserCombinators.ReadP`, `base:Data.List.NonEmpty`, `base:Data.Version`, `base:GHC.Maybe`                                                                                      |

Two things this list is not. It is **not** an estimate of what a working ShellCheck needs, because every count is a lower bound: a refused instance never revealed its own requirements, which is why clearing unboxed tuples *raised* the boundary from 450 to 592 and put `text` on it for the first time. And it is **not** a plan — the largest row is 272 refusals across three `base` modules of ordinary list code, and whether that is answered by widening the dump or by implementing the signatures is a decision the survey cannot make.
...
| cause                                         | refusals |
| --------------------------------------------- | -------: |
| a let binds a value whose type has no carrier |       79 |
| a let group binds several values at once      |        1 |
| **a recursive value binding**                 |    **0** |
| **a join point bound as a value**             |    **0** |

All 79 are monad-transformer types with no carrier — `WriterT` 44, `RWST` 12, `Monad` 10, `MonadState` 6 — and none of them is about recursion. A conflated message cannot be ranked, and until it was split the ranking could not say which of the three the program actually contains.
