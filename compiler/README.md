...
`mise run canary` runs a complete small pipeline: compile the real Haskell sources in `compiler/canary/` with GHC 9.6.7 and the format-6 plugin; lower the selected pure entries and every dependency; emit standalone Rust; compile with `rustc`; compare against the Haskell oracle. Both `-O1` and `-O0 -fmax-simplifier-iterations=0` check 2,066 entry/input combinations, each with optimized and overflow-checked Rust: 8,264 comparisons. Signed 64-bit boundaries exercise overflowing arithmetic. Standard output, standard error and successful exit are checked. Alongside scalar control flow and lazy bindings, the suite covers sums, products, nested constructor patterns, DEFAULT, case-binder reuse, strict/lazy fields, `Maybe Int` and finite `[Int]` values.
...
`compiler/canary/boxed_checks.rs` compiles against the actual generated Rust in both profiles. Its sixteen semantic tests use deferred/instrumented inputs to check unused arguments and constructor fields, unselected/nested/default branches, delayed calls, shared inputs, strict constructor fields, CAF cell identity and nested/shared lazy bindings. The embedded runtime's seven tests also execute in each of the sixteen generated modules (128 test executions per profile). The unsimplified profile requires `DelayBlock` for the five lazy examples, `LazyBinding` for `lazyLet`/`lazyNested`, and constructor/match operations in the algebraic producers/consumers. The directly forced `lazyStrictUse` example is already desugared to a case, so its test checks forcing rather than requiring a surviving let.
...
This is a **pure backend**, not ShellCheck code generation: monomorphic functions over `Int#`, boxed `Int` and supported algebraic values are emitted only with source-verified NIR and a complete dependency closure. Direct self/mutual recursion and saturated local functions/join points are supported. `LocalScope` records each definition and its body region; `CallLocal` passes explicit captures and arguments. Source verification independently checks lexical visibility, signatures, definition identity, capture order and complete source accounting. Scalar-result tail calls and CFG transfers use a dispatcher loop, including cross-function mutual recursion; non-tail calls and lifted-result recursion still use the native stack. Constructor type parameters may be instantiated with closed supported types; this is not general function specialization. The target must be 64-bit. GADTs/existentials, newtype casts, unlifted datatypes, unsupported field carriers, escaping local functions/partial application, recursive value/thunk graphs, ordinary unlifted lets, polymorphic function specialization and Haskell `IO` remain unsupported. Integer command-line parsing and printing are explicit test adapters, not translated Haskell `main`; algebraic values are internal, not CLI inputs/outputs. The canary checks that attempting to emit the Haskell IO entry fails without replacing already-generated code. M3 as a whole remains open.

The recursion extension adds self/mutual calls, non-tail tree recursion, list traversal, captured local loops, join points and lazy local arguments. Scalar tail loops run one million iterations in both Rust build modes. Both Core profiles require `LocalScope` and `CallLocal` in the local-loop NIR. This extends the counts above to **9,176 differential comparisons** and **17 semantic tests**, plus the embedded runtime tests (**136 test executions per profile**). Canonical NIR coverage is now **5,338 / 9,795 live bindings**, with 4,457 refusals and 3,957 dead bindings skipped (+3 lowered); `mise run lower:coverage` reuses the unchanged constructor-bearing dumps.
