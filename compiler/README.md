...
The canary passes **22,504 differential cases** and 28 forced-error and branch probes per profile.

### Results

Canonical NIR coverage per owner: **8,412 lowered / 1,383 refused / 3,957 dead**, against 8,265 / 1,530. The instance survey: **9,822 = 8,421 lowered + 1,401 refused**, against 9,821 = 8,273 + 1,548. 188 of the 190 sites lower; the other 2 are the `$fEqShell` refusals.

## Remaining runtime and external dependencies

`mise run lower:specialize` against the canonical `-O1` dump reports **431 refusals over 100 bindings in 35 modules**. The groups below are by the work each needs, not by package.
...
| **Primops with no implementation yet.**                                                                                                                 |       85 | `ghc-prim:GHC.Prim` 81 (`dataToTag#` 39, `reallyUnsafePtrEquality#` 21, `uncheckedIShiftRA#` 8, `leWord#` 5, `tagToEnum#` 3, `unsafeFreezeArray#` 2, `andI#`, `orI#`, `quotRemInt#`), `ghc-prim:GHC.Magic.runRW#` 4                                                                                              |
...
| **`Eq`/`Ord` for lists, tuples and `Int`.**                                                                                                             |       59 | `ghc-prim:GHC.Classes` (`$fOrdList_$s$ccompare1` 39, `$fOrdList_$ccompare` 5, `$fEqList_$s$c==1` 3, `eqInt` 3, `compareInt#` 2, seven more at 1)                                                                                                                                                                 |
...
| **`Integer`.** This backend has no carrier for arbitrary precision.                                                                                     |       10 | `ghc-bignum:GHC.Num.Integer` (`integerEq` 3, `integerAdd` 2, `integerGe` 2, three more at 1)                                                                                                                                                                                                                     |
...
Every count is a lower bound. A refused instance never revealed its own requirements, which is why clearing a blocker can raise other counts: after the list predicates, `map` rose from 24 to 29 and `System.Info.os` appeared, because code that stopped at `elem` now reaches further.
...
```
Refused: 1401 = 1042 at a closed signature + 359 at an open signature
Open signatures: 359 refused instances of 359 bindings quantified over types they were not given; 20 of those bindings were also requested at closed types
```

Only 7 quantified bindings lower at their open signature. For 339 of the 359, the survey never requested a closed instantiation. Some are artifacts of the rooting, but not all of them. `ShellCheck.CFGAnalysis`'s `ST` code is quantified over the state thread `s`, which `runST`'s rank-2 type never instantiates to a closed type, so no specialization can close it. Those are real blockers.
