...
| `crates/h2r-build`       | The extraction pipeline as a library. `extract` stages and compiles the program, its twelve libraries and the entry module with the plugin, and skips each one whose recorded input fingerprint still matches. `layer` is what the `hs-*` build scripts run. Each layer works in one directory under Cargo's build directory, which every profile and every cargo invocation shares, so a new build unit finds the extraction already done. `h2r extract` is a CLI over the same code.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `crates/hs-libraries`    | The Core of the twelve libraries ShellCheck uses, compiled with the plugin and embedded as `DUMPS`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `crates/hs-shellcheck`   | The Core of ShellCheck, embedded as `DUMPS`, and the package database `hs-entry` compiles against.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `crates/hs-entry`        | The Core of `ShellCheckEntry.hs`, the Haskell functions `rshellcheck` calls, embedded as `DUMPS`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `crates/shellcheck-core` | Emits those functions from the three layers' Core as crates and compiles them with the rustc settings of the profile being built. A crate is recompiled only when its source, a dependency or those settings change. The library re-exports `h2r_entry`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `crates/rshellcheck`     | ShellCheck's command line in Rust, built with clap over `shellcheck-core`. `cargo build --release` at the repository root builds it, extraction included, and needs GHC 9.6.7 and cabal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

## Usage

```sh
. ~/.ghcup/env                 # GHC 9.6.7 + cabal
cargo h2r extract program   # → compiler/core-json/*.core.json
cargo build --release        # → target/release/rshellcheck, extracting whatever is stale

cargo h2r stats compiler/core-json --per-module
cargo h2r binders compiler/core-json ShellCheck.Fixer
cargo h2r laziness compiler/core-json                      # the census
cargo h2r laziness compiler/core-json --module ShellCheck.Fixer --explain --thunks-only
cargo h2r show compiler/core-json ShellCheck.Fixer 1287    # Core at a node id
cargo h2r parsec compiler/core-json                        # prove Parsec's CPS roles
cargo h2r parsec compiler/core-json --module ShellCheck.Parser --explain
cargo h2r parsec compiler/core-json --cfg 8106       # one region's graph
cargo h2r parsec compiler/core-json --module ShellCheck.Parser --cfg-all --json
cargo h2r show compiler/core-json ShellCheck.Parser 141341   # + its proof
cargo h2r tuples compiler/core-json                        # tuple flows and fates
cargo h2r tuples compiler/core-json --module ShellCheck.Checks.Commands --explain
cargo h2r tuples compiler/core-json --verify    # the independent re-derivation
cargo h2r tuples compiler/core-json --module ShellCheck.Analytics --scalar 30892
cargo h2r tuples compiler/core-json --module ShellCheck.CFG --scalar-all --json
cargo h2r tuples compiler/core-json --boundaries  # can all the views be applied at once?
cargo h2r fields compiler/core-json                        # constructor fields: what is evaluated, and when
cargo h2r fields compiler/core-json --con OuterToken
cargo h2r lists compiler/core-json                         # list flows: when is a spine demanded, and how much
cargo h2r lists compiler/core-json --axioms                # the library demand-semantics table
cargo h2r lists compiler/core-json --module ShellCheck.ASTLib --explain
cargo h2r text compiler/core-json                          # which list flows are text, and what is done with them
cargo h2r text compiler/core-json --heads                  # the text-head table
cargo h2r text compiler/core-json --module ShellCheck.Formatter.GCC --explain
cargo h2r classops compiler/core-json                      # class-op dispatch: which instance, which method
cargo h2r classops compiler/core-json --class Show --explain
cargo h2r classops compiler/core-json --module ShellCheck.Fixer --json
cargo h2r verify-rep compiler/core-json          # re-derive every M2.3 verdict independently
cargo h2r dictflow compiler/core-json                      # whole-program dictionary flow and erasure
cargo h2r higher compiler/core-json                        # function-valued boundaries: can one representation serve each?
cargo h2r verify-m24 compiler/core-json   # re-derive every positive M2.4 verdict independently
cargo h2r verify-m24 compiler/core-json --explain # …listing every refusal one by one
cargo h2r verify-rep compiler/core-json --explain # …listing every refusal, plus the accounting and the M1 link
cargo h2r fields compiler/core-json --module ShellCheck.CFG --view 10329   # one construction, field by field
cargo h2r fields compiler/core-json --module ShellCheck.AST --view-all --json
cargo h2r lists compiler/core-json --module ShellCheck.ASTLib --view 1220  # one flow: cells, consumers, the facts
cargo h2r text compiler/core-json --module ShellCheck.Formatter.GCC --view 11
cargo h2r show compiler/core-json ShellCheck.AST 5293      # + its M2.3 footers
cargo h2r tuples compiler/core-json --module Main --boundaries --explain
cargo h2r show compiler/core-json ShellCheck.Checks.Commands 4714   # + its tuple proof
cargo h2r classops compiler/core-json --view 1154          # one dispatch site, laid out
cargo h2r classops compiler/core-json --view-all --module ShellCheck.Fixer --json
cargo h2r higher compiler/core-json --view 51239           # one boundary: producers, uses, the rule order
cargo h2r higher compiler/core-json --view-all --module ShellCheck.AST --json
cargo h2r m24 compiler/core-json             # M2.4's accounting, residual and cross-links
cargo h2r show compiler/core-json ShellCheck.Fixer 1154    # + its M2.4 footers
cargo h2r lower compiler/core-json --reachability          # M3a: what Main.main can reach
cargo h2r lower compiler/core-json --reachability --explain checkScript
cargo h2r lower compiler/core-json --reachability --json
cargo h2r lower compiler/core-json --reachability --m24-link
cargo h2r lower compiler/core-json --rules                 # the A0-A11 rule table
```
...
```sh
cargo h2r fields compiler/core-json
cargo h2r fields compiler/core-json --module ShellCheck.AST --explain
cargo h2r fields compiler/core-json --con OuterToken
cargo h2r lists compiler/core-json                         # list flows: when is a spine demanded, and how much
cargo h2r lists compiler/core-json --axioms                # the library demand-semantics table
cargo h2r lists compiler/core-json --module ShellCheck.ASTLib --explain
...
cargo h2r text compiler/core-json --heads                  # the text-head table
cargo h2r text compiler/core-json --module ShellCheck.Formatter.GCC --explain
cargo h2r fields compiler/core-json --json
```
...
```sh
cargo h2r lists compiler/core-json
cargo h2r lists compiler/core-json --axioms
cargo h2r lists compiler/core-json --module ShellCheck.ASTLib --explain
cargo h2r text compiler/core-json                          # which list flows are text, and what is done with them
cargo h2r text compiler/core-json --heads                  # the text-head table
cargo h2r text compiler/core-json --module ShellCheck.Formatter.GCC --explain
cargo h2r lists compiler/core-json --json
```
...
```sh
cargo h2r fields compiler/core-json --module ShellCheck.CFG --view 10329
cargo h2r fields compiler/core-json --module ShellCheck.AST --view-all --json
cargo h2r lists compiler/core-json --module ShellCheck.ASTLib --view 1220
cargo h2r text compiler/core-json --module ShellCheck.Formatter.GCC --view 11
cargo h2r show compiler/core-json ShellCheck.AST 5293       # + its M2.3 footers
```
...
```sh
cargo h2r classops compiler/core-json --view 1154
cargo h2r classops compiler/core-json --view-all --module ShellCheck.Fixer --json
cargo h2r higher compiler/core-json --view 51239
cargo h2r higher compiler/core-json --view-all --module ShellCheck.AST --json
cargo h2r show compiler/core-json ShellCheck.Fixer 1154      # + its M2.4 footers
cargo h2r m24 compiler/core-json                             # the whole milestone
```
...
Profile flags are defined in `crates/h2r-build/src/extract/program.rs`. These are distinct Core populations, so more removable tuples alone does not make a better input: the aggressive profiles also replicate substantially more thunk sites.
...
`mise run extract:libraries` runs `h2r extract library <package>` for each of them, which writes `compiler/library-json/<package>/`. `h2r lower --with <dir>` loads a library directory beside the program's, the `lower:*` tasks and the canary load all four that way, and the M1 to M3a reports keep measuring the program alone.
...
- **Interfaces.** `h2r extract interfaces` compares every rebuilt interface with the installed one, after removing hashes, dependent-file paths, the plugin dependency and the haddock section, and the extraction fails on any difference. That covers every exported unfolding, the `SrcLoc` paths inside them, and the Typeable fingerprints GHC derives from the unit id. All 359 interfaces are identical, `base`'s 274 included.
...
The [binary conformance harness](../crates/h2r-conformance/README.md) reuses the `rust-port` corpus extractor and fuzz generator. Run `mise run conformance --candidate <binary>` or `mise run conformance:fuzz --candidate <binary>`; the candidate is explicit and need not link to any Rust-port library.
...
`h2r extract matrix` extracts Core under six profiles and `h2r compare` puts the census side by side. The hypothesis was that `-fno-full-laziness` plus aggressive specialisation would remove float-outs, dictionaries and much of the memo population before any pass of ours runs.
